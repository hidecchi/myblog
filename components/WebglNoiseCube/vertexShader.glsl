...
layout(location = 2) in vec4 aPositionBox;
...
void main() {
    float noise = snoise(vec3(aPosition.x, aPosition.y, aPosition.z - uTick * 0.02f));
    vec3 ray = vec3(0.0f, 0.0f, 1.0f);
    float flesnel = abs(dot(aPosition.rgb, ray)) * 0.4f + 0.5f;
    vColor = vec4(flesnel, flesnel, flesnel, 1.0f);
    gl_Position = uProjection * uView * uModel * (aPosition + noise * 0.13f);
